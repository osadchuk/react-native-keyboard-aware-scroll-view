...
const KeyboardAwareMixin = {
  ...
  resetKeyboardSpace: function () {
    ...
    if (this.props.enableResetScrollToCoords === false) {
      ...
      const layout = this.scrollViewLayout;
      const overScrolledY = layout.height + contentOffset.y > contentSize.height;
      const overScrolledX = layout.width + contentOffset.x > contentSize.width;
      if (overScrolledX || overScrolledY) {
        this.scrollToEnd();
      }
      return
  ...
  scrollToFocusedInputWithNodeHandle: function (nodeID: number, extraHeight: number) {
    ...
    this.scrollToFocusedInput(reactNode, extraHeight + this.props.extraScrollHeight + this.yOffset)
  },
  ...
  handleOnContentSizeChange: function(width: number, height: number) {
    this.contentSize = {width, height}
  },

  handleOnLayout: function(event: Event) {
    this.yOffset = event.nativeEvent.layout.y;
    this.scrollViewLayout = event.nativeEvent.layout;
  },
